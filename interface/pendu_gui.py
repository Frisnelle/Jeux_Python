...
from style import (
    ...
    COULEUR_SUCCES, COULEUR_ERREUR, bouton_retour,
)
...
class FramePenduJeu(tk.Frame):
    def __init__(self, parent, app, categorie, params):
        ...
        self.label_essais.pack(pady=(0, 8))

        self.label_mot = tk.Label(
            self, text=self._mot_masque(), font=("Consolas", 22, "bold"),
            bg=COULEUR_FOND, fg=COULEUR_TITRE,
        )
        self.label_mot.pack(pady=8)

        conteneur_clavier = tk.Frame(self, bg=COULEUR_FOND)
        conteneur_clavier.pack(pady=(5, 10))

        for i, lettre in enumerate(ALPHABET):
            b = tk.Button(
                conteneur_clavier, text=lettre, font=("Segoe UI", 10, "bold"),
